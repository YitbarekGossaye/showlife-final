...
import portfolio_data from '@/data/portfolio-data'; // Import your portfolio data here
...
const BreadcrumbEight = ({ project }) => {
   ...
   // Find the project in portfolio_data based on its id
   const selectedProject = portfolio_data.find(p => p.id === project.id);

   if (!selectedProject) {
      return null; // Handle case where project is not found
   }
                        ...
                        <div className="col-xl-12">
                           <div className="breadcrumb__content z-index-3 mb-60">
                              <div className="breadcrumb__text wow tpfadeIn" data-wow-duration=".9s" data-wow-delay=".6s">
                                 <span>{selectedProject.title}</span> {/* Display dynamic title from portfolio data */}
                              </div>
                              <h3 ref={animeRef} className="breadcrumb__title anime_text">{selectedProject.title}</h3>
                           </div>
                        </div>
                     </div>
